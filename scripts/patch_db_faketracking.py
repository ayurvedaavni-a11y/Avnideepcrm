# -*- coding: utf-8 -*-
# Phase 2: db.ts + 0001_init.sql sync + fake tracking removal
...
# ============ 1) 0001_init.sql ============
p = 'worker/migrations/0001_init.sql'
c = load(p)
NL = '\r\n' if '\r\n' in c else '\n'
...
# ============ 2) db.ts Customer + Order interfaces ============
p = 'src/db/db.ts'
c = load(p)
NL = '\r\n' if '\r\n' in c else '\n'
...
# ============ 3) Fake tracking — OrderPipeline handleAdvanceOrder ============
...
# OrderDetailModal tracking display — 'Tracking ID not assigned yet'
c = rep(c, NL,
...
save(p, c)
print('OrderPipeline fake tracking removed')

# ============ 4) Fake tracking — Logistics ============
...
save(p, c)
print('Logistics fake tracking removed')

# ============ 5) DeliveredCustomers — tracking display ============
...
save(p, c)
print('DeliveredCustomers tracking display updated')

print('\n=== ALL PHASE 2 CHANGES APPLIED ===')